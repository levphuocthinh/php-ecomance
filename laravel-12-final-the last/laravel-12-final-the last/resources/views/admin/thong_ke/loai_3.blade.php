            ...
            <div class="card">
                ...
                <div class="card-body">
                    <table class="table table-bordered">
                        <thead>
                            <tr>
                                <th class="text-center align-middle text-nowrap">Mặt Hàng</th>
                                <th class="text-center align-middle text-nowrap">Tổng Số Lượng</th>
                                <th class="text-center align-middle text-nowrap">Chi Tiết</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr v-for="(v, k) in list">
                                <td>@{{ v.ten_san_pham }}</td>
                                <td class="text-center align-middle">@{{ v.tong_so_luong }}</td>
                                <td><button v-on:click="xemChiTiet(v)" class="btn btn-primary" data-bs-toggle="modal"
                                        data-bs-target="#chiTietModal">Xem</button></td>
                            </tr>
                        </tbody>
                    </table>
                                    ...
                                    <table class="table table-bordered">
                                        ...
                                        <tbody>
                                            <tr v-for="(v, k) in detail">
                                                <td>@{{ k + 1 }}</td>
                                                <td>@{{ v.ho_lot }} @{{ v.ten }}</td>
                                                <td>@{{ v.ma_don_hang }}</td>
                                                <td>@{{ v.ten_nguoi_nhan }}</td>
                                                <td>@{{ v.so_dien_thoai }}</td>
                                                <td class="text-center">@{{ v.so_luong_mua }}</td>
                                                <td>@{{ numberFormat(v.don_gia_mua) }}</td>
        ...
        new Vue({
            ...
            methods: {
                getData() {
                    ...
                    axios
                        .post('{{ Route('dataThongKe3') }}', payload)
                        .then((res) => {
                ...
                xemChiTiet(payload) {
                    ...
                    axios
                        .post('{{ Route('dataChiTietThongKe3') }}', payload)
                        .then((res) => {
        ...
        function getChart() {
            ...
            axios
                ...
                .then((res) => {
                    ...
                    du_lieu = res.data.tong_so_luong;
