                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-bordered">
                            <thead>
                                <tr>
                                    <th class="text-center align-middle text-nowrap">Khách Hàng</th>
                                    <th class="text-center align-middle text-nowrap">Đại Lý</th>
                                    <th class="text-center align-middle text-nowrap">Tổng Số Đơn Hàng</th>
                                    <th class="text-center">Action</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr v-for="(v, k) in list">
                                    <td class="text-center align-middle text-nowrap">@{{ v.ho }}
                                        @{{ v.dem }} @{{ v.ten }}</td>
                                    <td class="text-center align-middle text-nowrap">@{{ v.ho_partner }}
                                        @{{ v.ten_partner }}</td>
                                    <td class="text-center">@{{ v.tong_don_hang }}</td>
                                    <td>
                                        <button v-on:click="xemChiTiet(v)" data-bs-toggle="modal"
                                            data-bs-target="#chiTietModal" class="btn btn-primary">Xem</button>
                                    </td>
                                </tr>
                            </tbody>
                        </table>
    ...
    <script>
        new Vue({
            ...
            methods: {
                getData() {
                    ...
                    axios
                        .post('{{ Route('dataThongKe6') }}', payload)
                        .then((res) => {
                ...
                xemChiTiet(payload) {
                    ...
                    axios
                        .post('{{ Route('dataChiTietThongKe6') }}', payload)
                        .then((res) => {
        ...
        function getChart() {
            ...
            axios
                .post('{{ Route('dataThongKe6') }}', payload)
                .then((res) => {
                    ten_du_lieu = res.data.ho_va_ten_kh;
                    du_lieu = res.data.tong_don_hang;
